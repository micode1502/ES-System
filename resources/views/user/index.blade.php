<x-app-layout>
                    ...
                    <div class="card-body p-4">
                        @can('user-create')
                            <div class="flex justify-between items-center mb-4">
                                <button
                                    class="btn btn-primary cursor-pointer bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                                    id="btnAdd">
    ...
    <div id="modal" tabindex="-1" aria-hidden="true"
        class="hidden bg-opacity-60 bg-black overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="modal-content relative p-4 w-full max-w-xl md:max-w-3xl max-h-full">
        </div>
    </div>
    @push('scripts')
    {{ $dataTable->scripts() }}
    <script>
            ...
            if (op == 'edit') {
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        $("#modalTitle").text("{{ __('Actualizar Usuario') }}");
        ...
        $('#btnAdd').on('click', function() {
            $.ajax({
                ...
                success: function(res) {
                    $('#modal').find('.modal-content').html(res);
                    $("#modalTitle").text("{{ __('Registrar Usuario') }}");
